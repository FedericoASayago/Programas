var juego={
    ...
    var nuevoElemento = $('<div>');
        ...
        nuevoElemento.css({
          backgroundImage:'url(pieza/' + numero + '.jpeg)',
          top: fila * 200,
          left: columna * 200
        });
...
capturarTeclas(){
  ...
  iniciar:function(el,dific){
    this.instalarPiezas(el);
    this.mezclarFichas(dific);
    this.capturarTeclas();
     $("#ganar").hide();
  },
}
function start (){
 $("#principal").hide();
 $("#todo").css("background-image", "url(Estilo/Voldemort.jpg")
 $("#nivel1").show();
  var $juego= $("#juego");
  juego.iniciar($juego, $('#principal input[type=radio]:checked').val());
}